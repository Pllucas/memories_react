...
const toggloeFavorite = async(req, res) => {
    try {
        const memory = await Memory.findById(req.params.id)
...
const addComent = async(req,res) => {
    try {
       ...
       await memory.save()
        ...
        res.json({msg: "Comentário adicionando", memory})
    } catch (error) {
        console.log(error)
        res.status(500).send("Ocorreu um erro!")
    }
...
module.exports = { createMemory, getMemories, getMemory, deleteMemory, updateMemory, toggloeFavorite, addComent }